...
```{r}
library(GPArotation)
resfa_2007 <- fa(Data2007[,c(2:7)],nfactors= 4,cor='mixed', rotate='varimax', fm='minres')
```
...
```{r}
resfa_2017 <- fa(Data2017[,c(2:7)],nfactors= 2,cor='mixed', rotate='varimax', fm='minres')
```
...
```{r}
edu_resfa_2017 <- fa(Data2017[,c(5,6)],nfactors = 1,cor = 'mixed',rotate = "varimax",fm="minres")
```


```{r}
fa.diagram(edu_resfa_2017)
```
```{r}
model_2017='Educacion =~ CompSecEdu + EduYears'
Data2017[,c(5,6)]=scale(Data2017[,c(5,6)])
library(lavaan)
```

```{r}
cfa_fit_2017 <- cfa(model_2017, data=Data2017[,c(5,6)], 
           std.lv=TRUE,  
           missing="fiml")
```

```{r}
allParamCFA_2017=parameterEstimates(cfa_fit_2017,standardized = T)
allFitCFA_2017=as.list(fitMeasures(cfa_fit_2017))
```

```{r}
library(knitr)
kable(allParamCFA_2017[allParamCFA_2017$op=="=~",])
```

```{r}
allFitCFA_2017[c("chisq", "df", "pvalue")] # pvalue>0.05
```

```{r}
allFitCFA_2017$tli # > 0.90
```

```{r}
allFitCFA_2017[c('rmsea.ci.lower','rmsea' ,'rmsea.ci.upper')] # 0.05 en el Int de Conf?
```

```{r}
Data2017=as.data.frame(cbind(Data2017,lavPredict(cfa_fit_2017)))
```

Creando 2007

```{r}
edu_resfa_2007 <- fa(Data2007[,c(5,6)],nfactors = 1,cor = 'mixed',rotate = "varimax",fm="minres")
```

```{r}
fa.diagram(edu_resfa_2007)
```

```{r}
model_2007='Educacion =~ CompSecEdu + EduYears'
Data2007[,c(5,6)]=scale(Data2007[,c(5,6)])
```

```{r}
cfa_fit_2007 <- cfa(model_2007, data=Data2007[,c(5,6)], 
           std.lv=TRUE,  
           missing="fiml")
```

```{r}
allParamCFA_2007=parameterEstimates(cfa_fit_2007,standardized = T)
allFitCFA_2007=as.list(fitMeasures(cfa_fit_2007))
```

```{r}
kable(allParamCFA_2007[allParamCFA_2007$op=="=~",])
```

```{r}
allFitCFA_2007[c("chisq", "df", "pvalue")] # pvalue>0.05
```

```{r}
allFitCFA_2007$tli # > 0.90
```
```{r}
allFitCFA_2007[c('rmsea.ci.lower','rmsea' ,'rmsea.ci.upper')] # 0.05 en el Int de Conf?
```

```{r}
Data2007=as.data.frame(cbind(Data2007,lavPredict(cfa_fit_2007)))
```
...
library(cluster)
library(factoextra)
g.dist2007 = daisy(Data2007[,-c(1,6)], metric="gower")
g.dist2017 = daisy(Data2017[,-c(1,6)], metric="gower")
```


```{r}
Data2007[,7]= as.numeric(Data2007[,7])
str(Data2007)
```

```{r}
fviz_nbclust(Data2007[,-c(1,6)], pam,diss=g.dist,method = "gap_stat",k.max = 10,verbose = F)

```

```{r}
fviz_nbclust(Data2007[,-c(1,6)], hcut,diss=g.dist,method = "gap_stat",k.max = 10,verbose = F)
```

#ahora con la data 2017

```{r}
Data2017[,7]=as.numeric(Data2017[,7])
str(Data2017)
```


```{r}
fviz_nbclust(Data2017[,-c(1,6)], pam,diss=g.dist,method = "gap_stat",k.max = 10,verbose = F)

```


```{r}
fviz_nbclust(Data2017[,-c(1,6)], hcut,diss=g.dist,method = "gap_stat",k.max = 10,verbose = F)
```
...
```{r}
library(fpc)
db.cmd2007 = dbscan(g.dist.cmd2007, eps=0.075, MinPts=5,method = 'dist')
db.cmd2017 = dbscan(g.dist.cmd2017, eps=0.11, MinPts=5,method = 'dist')
```
...
limites2007=c(-0.75,0.41)
limites2017=c(-0.75,0.47)
```


```{r}
library(ggrepel)
base2007= ggplot(Data2007[,-c(1,6)],aes(x=dim1, y=dim2)) + ylim(limites2007) + xlim(limites2007) + coord_fixed()
dbplot2007= base2007 + geom_point(aes(color=dbCMD)) 
dbplot2007
```
```{r}
base2017= ggplot(Data2017[,-c(1,6)],aes(x=dim1, y=dim2)) + ylim(limites2017) + xlim(limites2017) + coord_fixed()
dbplot2017= base2017 + geom_point(aes(color=dbCMD)) 
dbplot2017
```

```{r}
row.names(Data2007)=Data2007$Code

dbplot2007 + geom_text_repel(size=2, aes(label=rownames(Data2007)))
```
```{r}
db.cmd2007
#Perdido PunoPuno
...
```{r}
row.names(Data2017)=Data2017$Code

dbplot2017 + geom_text_repel(size=2, aes(label=rownames(Data2017)))
#Perdidos: San Antonio de Putina-Puno/Huancavelica-Huancavelica/Huamanga-Ayacucho/Sechura-Piura/Puno-Puno
```
