...
```{r}
Edu=Data2017[,c(5,6)]
Eduresfa <- fa(Edu,nfactors = 1,cor = 'mixed',rotate = "varimax",fm="minres")
```
...
```
```{r}
model='Educación =~ CompSecEdu + EduYears'
Data2017[,c(5,6)]=scale(Data2017[,c(5,6)])
library(lavaan)
```

```{r}
cfa_fit <- cfa(model, data=Data2017, 
           std.lv=TRUE,  
           missing="fiml")
```
...
```
```{r}
Data2017=as.data.frame(cbind(Data2017,lavPredict(cfa_fit)))
```
