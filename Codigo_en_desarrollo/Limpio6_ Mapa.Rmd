...
Map <- rgdal::readOGR('https://github.com/PoliticaygobiernoPUCP/estadistica_anapol2/raw/master/PER_adm2.json',stringsAsFactors = FALSE)
```

```{r}
plot(Map, border='black')
```

```{r}
sort(Map@data$NAME_2)
```

```{r}
library(stringi)
Map$NAME_2=stri_trans_general(str=Map$NAME_2, id="Latin-ASCII")
```

```{r}
Map@data$NAME_2=paste0(Map@data$NAME_1,Map@data$NAME_2)
Map$NAME_2=stri_trans_general(str=Map$NAME_2, id="Latin-ASCII")
```

```{r}
Mapa2007=merge(Map@data,Data2007,by.x='NAME_2',by.y='Prov_Name',all.x=T,all.y=T)
Mapa2007$id=NULL
Mapa2007[!complete.cases(Mapa2007),]
```
```{r}
Mapa2007=na.omit(Mapa2007)
names(Mapa2007)
```

```{r}
Data2007$Cluster=as.numeric(Data2007$Cluster)
Data2007= data.frame(Data2007$Prov_Name,Data2007$Urb_Porc,Data2007$Inter_Porc,Data2007$LifeHope,Data2007$CompSecEdu,Data2007$EduYears,Data2007$IncFamPPC,Data2007$Code,unlist(Data2007$Cluster))

names(Data2007)=c('Prov_Name','Urb_Porc','Inter_Porc','LifeHope','CompSecEdu','EduYears','IncFamPPC','Code','Cluster')

str(Data2007)

aggregate(Data2007~Cluster,data = Mapa2007,FUN = mean)
```
