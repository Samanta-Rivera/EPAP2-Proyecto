...
plot(Map2007,col='black', main='Voto a la derecha 2014 \n usando clusters')
...
```{r}
myColors2=c("blue","red","yellow")
Map2017@data$ClusterDI=recode(Map2017@data$ClusterDI,"1=2;2=1;3=3")
```

```{r}
plot(Map2017,col='black', main='Voto a la derecha 2018 \n usando clusters')
plot(Map2017,col=myColors2[Map2017$ClusterDI],add=T,border=NA)
```
...
```{r}
checkReg2007=as.data.frame(influence.measures(reg2007)$is.inf)
head(checkReg2007)
```
...
```
```{r}
checkReg2017=as.data.frame(influence.measures(reg2017)$is.inf)
head(checkReg2017)
```
...
VD=c('Urb_Porc','Inter_Porc', 'LifeHope', 'CompSecEdu', 'EduYears', 'IncFamPPC','VOTODERECHA')
regData_2007=Data2007_novote[,VD]
modelBeta_2007=betareg(VOTODERECHA~.,data=regData_2007)
summary(modelBeta_2007)
```

```{r}
regData_2017=Data2017_novote[,VD]
modelBeta_2017=betareg(VOTODERECHA~.,data=regData_2017)
summary(modelBeta_2017)
```